...
TMP_ZIP="/tmp/nezha-agent_linux_${ARCH}.zip"

info "下载 nezha-agent (linux/${ARCH})..."
...
# ── 安装二进制 ────────────────────────────────────────────────
mkdir -p "$AGENT_DIR"
unzip -qo "$TMP_ZIP" -d "$AGENT_DIR"
rm -f "$TMP_ZIP"

# 二进制：root 拥有，任何人可执行，nezha-agent 无法替换或删除
...
# root 拥有，nezha-agent 只读，其他人不可见
chown root:"$AGENT_USER" "$CONFIG_FILE"
chmod 640 "$CONFIG_FILE"
ok "config.yml 已写入并锁定（nezha-agent 只读）"

# 目录本身：root 拥有，nezha-agent 无写权限
...
echo "══════════════════════════════════════════════════════"
echo ""
printf "  %-20s %s\n" "查看状态："  "systemctl status nezha-agent"
printf "  %-20s %s\n" "实时日志："  "journalctl -u nezha-agent -f"
echo ""
